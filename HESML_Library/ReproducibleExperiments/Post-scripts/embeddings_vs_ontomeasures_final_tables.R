...
inputDir = "D:/Basura/"
outputDir = "D:/Basura/ProcessedOutputFiles/"
...
table_Spearman_SimDatasets<-matrix(nrow = nrow(table_Pearson_SimDatasets), ncol = ncol(table_Pearson_SimDatasets))

# We set the names of rows and columns

colnames(table_Spearman_SimDatasets)<-colnames(table_Pearson_SimDatasets)
rownames(table_Spearman_SimDatasets)<-rownames(table_Pearson_SimDatasets)

table_Harmonic_SimDatasets<-matrix(nrow = nrow(table_Pearson_SimDatasets), ncol = ncol(table_Pearson_SimDatasets))

colnames(table_Harmonic_SimDatasets)<-colnames(table_Pearson_SimDatasets)
rownames(table_Harmonic_SimDatasets)<-rownames(table_Pearson_SimDatasets)
...
table_Pearson_RelDatasets<-matrix(nrow = ncol(rawdata_MC28) - 2, ncol = length(rawdataRelNounDatasets))
...
table_Spearman_RelDatasets<-matrix(nrow = nrow(table_Pearson_RelDatasets), ncol = ncol(table_Pearson_RelDatasets))

colnames(table_Spearman_RelDatasets)<-colnames(table_Pearson_RelDatasets)
rownames(table_Spearman_RelDatasets)<-rownames(table_Pearson_RelDatasets)

table_Harmonic_RelDatasets<-matrix(nrow = nrow(table_Pearson_RelDatasets), ncol = ncol(table_Pearson_RelDatasets))

colnames(table_Harmonic_RelDatasets)<-colnames(table_Pearson_RelDatasets)
rownames(table_Harmonic_RelDatasets)<-rownames(table_Pearson_RelDatasets)
...
table_Spearman_allEmbeddings<-matrix(ncol = ncol(table_Pearson_allEmbeddings), nrow = nrow(table_Pearson_allEmbeddings))
table_Harmonic_allEmbeddings<-matrix(ncol = ncol(table_Pearson_allEmbeddings), nrow = nrow(table_Pearson_allEmbeddings))

nOntologyBasedMeasures = 21;

rownames(table_Pearson_allEmbeddings)<-rawdataAllDatasetNames
colnames(table_Pearson_allEmbeddings)<-colnames(rawdata_WS353Full)[3:ncol(rawdata_WS353Full)]

rownames(table_Spearman_allEmbeddings)<-rownames(table_Pearson_allEmbeddings)
colnames(table_Spearman_allEmbeddings)<-colnames(table_Pearson_allEmbeddings)

rownames(table_Harmonic_allEmbeddings)<-rownames(table_Pearson_allEmbeddings)
colnames(table_Harmonic_allEmbeddings)<-colnames(table_Pearson_allEmbeddings)
...
# PERFORMANCE of word emebddings only in SIMILARITY DATASETS
...
for (iMeasure in 1:nrow(table_pvalues_AttractReppel_SimDatasets))
{
	...
	table_pvalues_AttractReppel_SimDatasets[iMeasure, 2] = round(t.test(table_Pearson_SimDatasets[1,], table_Pearson_SimDatasets[iMeasure + 1,], paired = TRUE,alternative="greater")$p.value, digits=5)
	table_pvalues_AttractReppel_SimDatasets[iMeasure, 3] = rownames(table_Spearman_SimDatasets)[iMeasure + 1]
	table_pvalues_AttractReppel_SimDatasets[iMeasure, 4] = round(t.test(table_Spearman_SimDatasets[1,], table_Spearman_SimDatasets[iMeasure + 1,], paired = TRUE,alternative="greater")$p.value, digits=5)
	table_pvalues_AttractReppel_SimDatasets[iMeasure, 5] = rownames(table_Harmonic_SimDatasets)[iMeasure + 1]
	table_pvalues_AttractReppel_SimDatasets[iMeasure, 6] = round(t.test(table_Harmonic_SimDatasets[1,], table_Harmonic_SimDatasets[iMeasure + 1,], paired = TRUE,alternative="greater")$p.value, digits=5)
}
...
# We create a list with datasets including ontology-based measures and word embeddings
...
averaged_Pearson_results<-matrix(nrow = nrow(table_Pearson_allEmbeddings) - 1, ncol = choose(nWordModels, 2))
rownames(averaged_Pearson_results)<-rownames(table_Pearson_allEmbeddings)[1:nrow(table_Pearson_allEmbeddings) - 1]
colnames(averaged_Pearson_results)<-c(1:ncol(averaged_Pearson_results))

averaged_Spearman_results<-matrix(nrow = nrow(averaged_Pearson_results), ncol=ncol(averaged_Pearson_results))
rownames(averaged_Spearman_results)<-rownames(averaged_Pearson_results)
colnames(averaged_Spearman_results)<-c(1:ncol(averaged_Pearson_results))

averaged_Harmonic_results<-matrix(nrow = nrow(averaged_Pearson_results), ncol=ncol(averaged_Pearson_results))
rownames(averaged_Harmonic_results)<-rownames(averaged_Pearson_results)
colnames(averaged_Harmonic_results)<-c(1:ncol(averaged_Pearson_results))
...
for (iDataset in 1:length(rawdata_onlyEmbeddings_datasets))
{
	...
	# Obtengo el número de embeddings

	nEmbeddings = ncol(rawdata) - 2

	# We create the matrix containing all raw similarity values for all combinations

	raw_averaged_similarity_matrix = matrix(nrow = nrow(rawdata), ncol = 2 + ncol(averaged_Pearson_results))
	...
	# We compute all averaged similarity values
	...
	for (i in 1:(nEmbeddings - 1))
	{
		for (j in (i + 1):nEmbeddings)
		{
			...
			averaged_Pearson_results[iDataset, iModelPair] = cor(rawdata[,2], averaged_sim, method = "pearson")
			averaged_Spearman_results[iDataset, iModelPair] = cor(rawdata[,2], averaged_sim, method = "spearman")
			averaged_Harmonic_results[iDataset, iModelPair] = 2 * averaged_Pearson_results[iDataset, iModelPair] * averaged_Spearman_results[iDataset, iModelPair] / (averaged_Pearson_results[iDataset, iModelPair] + averaged_Spearman_results[iDataset, iModelPair])			

			colnames(averaged_Pearson_results)[iModelPair]<-colnames(raw_averaged_similarity_matrix)[iModelPair + 2]
			colnames(averaged_Spearman_results)[iModelPair]<-colnames(raw_averaged_similarity_matrix)[iModelPair + 2]
			colnames(averaged_Harmonic_results)[iModelPair]<-colnames(raw_averaged_similarity_matrix)[iModelPair + 2]
